...
bool_t readKeyFalling ( GPIO_InitTypeDef* GPIOx, uint16_t GPIO_Pin){
...
bool_t readKeyRising ( GPIO_InitTypeDef* GPIOx, uint16_t GPIO_Pin){

	debounceFSM_t *pBtn;

	pBtn = btn;

	if( pBtn->btnRising == true){
		pBtn->btnRising = false;
		return true;
	}
	else
		pBtn->btnRising = false;
