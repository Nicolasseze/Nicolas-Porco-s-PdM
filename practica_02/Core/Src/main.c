...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	// Defino variables para el retardo del led
	delay_t delayTime;
	tick_t ledDuration[] = {1000, 200, 100 };
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  //Configuro el retardo del led en 100ms
  delayInit( &delayTime, ledDuration[0] );
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  for( uint8_t i = 0; i < 3; i++ ){ // 3 patrones de parpadeo
		  delayWrite(&delayTime, ledDuration[i] / 2); //Configura el tiempo de parpadeo /2 DC = 50%

		  uint8_t j = 0;
		  while( j < 5 * 2 ){ // 5 ciclos On/Off
			  if( delayRead( &delayTime ) ){
